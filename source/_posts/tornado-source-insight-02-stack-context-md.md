...
放到IOLoop中，在适当的时机调用。而在do_cb调用的时候早就不在try...except块中。那么怎么才能做到统一的处理的，可以将try...except
部分封装起来，像下面这样。
...
class StackContext(object):
    ...
    #这个方法在__enter__中返回， 如果你不想这个StackContext往里传播，就应该with StackContext(some_method) as deactivate, 然后在with块
    #内部调用deactivate方法。
...
def wrap(fn):
    ...
    #这个变量很关键，是wrap时的contexts。为啥要用list来装_state.contexts而不直接赋值呢？
    #其实很简单，因为python2.x中只能访问闭包中的变量不能修改。知道python3.x才有nonlocal关键字。
    #把_state.contexts放到列表里，列表本身不会被修改，但是列表中的内容却可以修改，tornado其他代码也用了此技术。
    ...
    def wrapped(*args, **kwargs):
        ret = None
        try:
            #这个是函数fn执行的是contexts，也就是当前的contexts与上面的cap_contexts不是一回事，一个是执行闭包时，一个是生成闭包时
            #注意这个两个时间的不同
            current_state = _state.contexts

            #删除那些deactivate被调用过的StackContext。前文也说过可以用deactivate取消StackContext的传播。
            #_remove_deactivated再下面单讲
...
def _remove_deactivated(contexts):
    ...
    #那个大链表中的未激活的StackContext，下面的操作就是清理那个大链表
...
好了，基本的都说的差不多了。ExceptionContext就是StackContext的简化版本，有兴趣的童鞋可以去看看，其实Tornado中用ExceptionContext用的更多一点，因为
